...
import com.android.imageloader.Constants;
...
public class UILWidgetProvider extends AppWidgetProvider {
	...
	static void updateAppWidget(Context context, final AppWidgetManager appWidgetManager, final int appWidgetId) {
		...
		ImageLoader.getInstance().loadImage(context, Constants.IMAGES[0], minImageSize, optionsWithFakeDisplayer, new SimpleImageLoadingListener() {
			@Override
		...
		ImageLoader.getInstance().loadImage(context, Constants.IMAGES[1], minImageSize, optionsWithFakeDisplayer, new SimpleImageLoadingListener() {
			@Override
			public void onLoadingComplete(Bitmap loadedImage) {
